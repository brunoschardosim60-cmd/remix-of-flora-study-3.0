...
export default function AnamnesisSimulator({ level, onLearningEvent }: { level: MedicineLevel; onLearningEvent?: (event: MedicineLearningEvent) => void }) {
  ...
  const { speak, stop: stopVoice, playing: voicePlaying, loading: voiceLoading } = useFloraVoice();
  ...
  return <div className="med-page med-anamnesis-page">
    <header className="med-page-heading med-anamnesis-heading"><div><span className="med-eyebrow">CONSULTA SIMULADA · {level === "beginner" ? "INICIANTE" : level === "basic" ? "CICLO BÁSICO" : level === "clinical" ? "CICLO CLÍNICO" : level.toUpperCase()}</span><h1>Anamnese imersiva</h1><p>Converse livremente. O paciente responde somente com os fatos clínicos validados deste caso.</p></div><div className="med-anamnesis-mode"><MessageCircle /><span><strong>Paciente dinâmico e ancorado</strong><small>IA para compreender a pergunta; dados estruturados para responder.</small></span></div></header>
    ...
    <div ref={workspaceRef} className={`med-anamnesis-workspace ${isFullscreen ? "is-fullscreen" : ""}`}>
      ...
      <main className="med-anamnesis-conversation">
        {clinicalCase.sensitive && !sensitiveAccepted ? <section className="med-anamnesis-sensitive-gate"><AlertTriangle /><span>CONTEÚDO SENSÍVEL</span><h2>Esta entrevista exige cuidado</h2><p>O caso aborda {clinicalCase.sensitiveWarnings?.join(", ")}. As respostas são fictícias, não gráficas e podem causar desconforto.</p><button onClick={() => { setSensitiveAccepted(true); void speak(clinicalCase.openingStatement, "padrao"); }}><Check /> Estou ciente — iniciar entrevista</button><small>Você pode escolher outro caso na biblioteca acima.</small></section> : evaluated ? <AnamnesisReport clinicalCase={clinicalCase} questions={askedQuestions} decision={selectedDecision!} summary={summary} score={score} missedCritical={missedCritical} sources={sourceList} crisisOccurred={crisisActive} crisisResolved={crisisResolved} onRestart={() => resetSession()} /> : <>
          ...
          <div className="med-anamnesis-composer">{interactionError && <p role="status"><AlertTriangle /> {interactionError}</p>}<div><textarea value={input} onChange={(event) => setInput(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); void sendMessage(); } }} placeholder="Pergunte livremente ao paciente..." aria-label="Pergunta ao paciente" disabled={isResponding || isTranscribing} /><button className={`mic ${isRecording ? "recording" : ""}`} onClick={toggleRecording} disabled={isTranscribing || isResponding} aria-label={isRecording ? "Parar gravação" : "Falar pergunta"}>{isTranscribing ? <Sparkles /> : isRecording ? <CircleStop /> : <Mic />}</button><button className="send" onClick={() => void sendMessage()} disabled={input.trim().length < 2 || isResponding || isTranscribing} aria-label="Enviar pergunta"><Send /></button></div><footer><span>{isRecording ? "Gravando — toque para parar" : isTranscribing ? "Transcrevendo sua fala..." : "Enter envia · Shift+Enter quebra a linha"}</span><button onClick={() => voicePlaying ? stopVoice() : void speak(messages.filter((m) => m.role === "patient").at(-1)?.text ?? clinicalCase.openingStatement, "padrao")} disabled={voiceLoading}>{voicePlaying ? <MicOff /> : <Volume2 />} {voiceLoading ? "Gerando voz..." : voicePlaying ? "Parar voz" : "Ouvir paciente"}</button></footer></div>
