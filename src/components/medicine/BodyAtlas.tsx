          ...
          <div className="med-layer-list">
            {bodyLayers.map((layer) => {
              ...
              return <button key={layer.id} className={activeLayer === layer.id ? "active" : ""} onClick={() => selectLayer(layer.id)}>
                ...
                <b aria-label={`${statistics.inCurrentView} estruturas nesta vista`} title={`${statistics.inCurrentView} estruturas disponíveis na vista ${view}; ${statistics.displayed} marcadores visíveis na imagem; ${statistics.catalogued} catalogadas`}><strong>{statistics.inCurrentView}</strong><small>nesta vista</small><span className="sr-only" aria-label={`${statistics.displayed} estruturas visíveis na imagem`} /></b>
              </button>;
            })}
          </div>
