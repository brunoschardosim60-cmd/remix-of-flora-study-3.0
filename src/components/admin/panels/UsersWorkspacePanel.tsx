...
import { Loader2, RefreshCcw, Save, RotateCcw, PlusCircle, Clock3, BookOpen, Trash2, Calendar } from "lucide-react";
    ...
    <div className="grid gap-4 lg:grid-cols-[320px_minmax(0,1fr)]">
      <section className="space-y-3 rounded-2xl border border-border bg-card/70 p-3">
        <div className="flex items-center justify-between">
          ...
          <span className="text-xs text-muted-foreground">{filteredUsers.length}/{users.length}</span>
        </div>
        <Input
            ...
            {filteredUsers.map((item) => (
              ...
              >
                <div className="flex items-center justify-between gap-2">
                  <p className="font-medium truncate">{item.displayName || "Sem nome"}</p>
                  {item.isAdmin ? (
      ...
      <section className="space-y-4">
        {!selectedUserId || !selectedUserCard ? (
          <div className="rounded-2xl border border-border bg-card/70 p-6 text-sm text-muted-foreground">
            Escolha um usuário para abrir o cofre seguro.
          </div>
          ...
          <>
            <div className="grid gap-3 md:grid-cols-4">
              <div className="rounded-2xl border border-border bg-card/70 p-4">
                ...
                <p className="mt-1 font-semibold truncate">{workspace.profile.display_name}</p>
              </div>
              <div className="rounded-2xl border border-border bg-card/70 p-4">
              ...
              <div className="space-y-4">
                <div className="rounded-2xl border border-border bg-card/70 p-4">
                  <div className="mb-3 flex items-center gap-2">
                    ...
                    <p className="font-medium">Perfil e correção manual</p>
                  </div>
                  <div className="grid gap-3 md:grid-cols-2">
                    <div className="space-y-2">
                      <label className="text-sm font-medium" htmlFor="adm-name">Nome exibido</label>
                      <Input id="adm-name" value={displayName} onChange={(e) => setDisplayName(e.target.value)} />
                      <Button
                    ...
                    <div className="space-y-2">
                      <label className="text-sm font-medium" htmlFor="adm-snap">Criar snapshot</label>
                      <Textarea
                        id="adm-snap"
                ...
                <div className="rounded-2xl border border-border bg-card/70 p-4">
                  ...
                  <div className="grid gap-3 md:grid-cols-3">
                    <div className="space-y-2">
                      <label className="text-sm font-medium" htmlFor="adm-hours">Horas</label>
                      <Input id="adm-hours" value={hoursToAdd} onChange={(e) => setHoursToAdd(e.target.value)} type="number" step="0.5" min="0" />
                    </div>
                    <div className="space-y-2">
                      <label className="text-sm font-medium" htmlFor="adm-date">Data</label>
                      <div className="flex items-center gap-2">
                        <Calendar className="h-4 w-4 text-muted-foreground" />
                        <Input id="adm-date" type="date" value={hoursDate} onChange={(e) => setHoursDate(e.target.value)} />
                      </div>
                    </div>
                    <div className="space-y-2">
                    ...
                    <div className="space-y-2 md:col-span-3">
                      <label className="text-sm font-medium" htmlFor="adm-note">Observação</label>
                      <Textarea
                        id="adm-note"
                        value={hoursNote}
                        onChange={(e) => setHoursNote(e.target.value)}
                        placeholder="Ex: recuperei horas perdidas após reclamação do usuário"
                      />
                    </div>
                  </div>
                  <Button
                    ...
                    onClick={() => void withSaving(
                      ...
                      `Horas adicionadas em ${hoursDate} com backup automático.`
                    )}
                ...
                <div className="rounded-2xl border border-border bg-card/70 p-4">
                  ...
                  <div className="grid gap-3 md:grid-cols-2">
                    <div className="space-y-2">
                      <label className="text-sm font-medium" htmlFor="adm-tema">Título do tema</label>
                      <Input id="adm-tema" value={topicTitle} onChange={(e) => setTopicTitle(e.target.value)} placeholder="Ex: Funções do 2º grau" />
                    </div>
                    <div className="space-y-2">
                      <label className="text-sm font-medium">Matéria</label>
                    ...
                    <div className="space-y-2 md:col-span-2">
                      <label className="text-sm font-medium" htmlFor="adm-tema-notes">Notas</label>
                      <Textarea id="adm-tema-notes" value={topicNotes} onChange={(e) => setTopicNotes(e.target.value)} />
                    </div>
                  </div>
                  <Button
                    className="mt-3"
                    onClick={() => void withSaving(
                      ...
                      "Tema adicionado com backup automático."
                    )}
                ...
                <div className="rounded-2xl border border-border bg-card/70 p-4">
                  <div className="mb-3 flex items-center gap-2">
                    ...
                    <p className="font-medium">Temas do usuário</p>
                  </div>
                  <div className="space-y-2">
                      ...
                      <div key={topic.id} className="rounded-xl border border-border bg-background p-3">
                        <div className="flex items-center justify-between gap-2">
                          <div className="min-w-0 flex-1">
                            <p className="font-medium truncate">{topic.tema}</p>
                  ...
                  {showSessions && (
                    <div className="max-h-[400px] space-y-3 overflow-y-auto">
                      {(() => {
                        ...
                        let shown = 0;
                        return Array.from(grouped.entries()).map(([day, daySessions]) => {
                ...
                <div className="space-y-3 max-h-[60vh] overflow-y-auto">
                  {snapshots.length === 0 ? (
                    <p className="text-sm text-muted-foreground">
                      Ainda não há snapshots para este usuário. Salve um antes de editar.
                    </p>
                  ) : (
...
// Re-export type to avoid orphan imports
