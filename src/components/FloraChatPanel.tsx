...
function parseFloraActions(text: string): { cleanText: string; actions: FloraAction[] } {
  ...
  while ((match = actionTokenRegex.exec(text)) !== null) {
    ...
    if (braceStart !== -1 && braceStart <= afterToken + 2) {
      ...
      if (extracted) {
        // JSON completo — parseia a ação e marca para remoção
        try {
        ...
        } catch { /* malformed JSON — remove silenciosamente */ }
      }
      // Se JSON incompleto (ainda chegando via stream), não remove nada agora
      // A limpeza final acontece quando o stream terminar e o texto estiver completo
    } else {
      // Token sem JSON associado — remove só o token
      removals.push({ start: match.index, end: afterToken });
    }
  }
  for (let i = removals.length - 1; i >= 0; i--) {
    cleanText = cleanText.slice(0, removals[i].start) + cleanText.slice(removals[i].end);
  }
  // Remove tokens de ação que ficaram sem JSON (streaming incompleto visível no chat)
  cleanText = cleanText
    .replace(/\[AÇÃO:[A-Z_]+\]\s*\{[\s\S]*$/g, "") // token + JSON incompleto no final
    .replace(/\[AÇÃO:[A-Z_]+\]/g, "")              // token sem JSON
