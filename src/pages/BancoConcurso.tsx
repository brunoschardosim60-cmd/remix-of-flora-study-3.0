...
function loadIaSession(): IaSessionSnapshot | null {
  try {
    const raw = sessionStorage.getItem(IA_SESSION_KEY);
    if (!raw) return null;
    const snap = JSON.parse(raw) as IaSessionSnapshot;
    // Expira após 6h
    if (!snap.createdAt || Date.now() - snap.createdAt > 6 * 60 * 60 * 1000) {
...
export default function BancoConcurso() {
  ...
  useEffect(() => {
    (async () => {
      ...
      // Restaura sessão IA do sessionStorage (sobrevive refresh)
      const snap = loadIaSession();
      if (snap) {
