      ...
      {mode === "draw" && (
        <div className={`transition-transform duration-300 ${expandedEditor ? "group/toolbar hover:translate-y-0 -translate-y-full absolute top-0 left-0 right-0 z-50" : ""}`}>
          <DrawingToolbar
            ...
            onAddStickyNote={(note) => handleStickyNotesChange([...drawingState.stickyNotes, note])}
            onSolveSelection={handleSolveSelection}
            onAutoSolve={handleAutoSolve}
            solvingMath={solvingMath}
            mathStatus={mathStatus}
            lastMathSuggestion={lastMathSuggestion}
            onSaveMathAsNote={handleSaveMathAsNote}
            onSaveMathAsFlashcard={handleSaveMathAsFlashcard}
            onSaveMathAsExercise={handleSaveMathAsExercise}
          />
        </div>
      )}

      <ShareNotebookDialog open={shareDialogOpen} onOpenChange={setShareDialogOpen} notebookId={id!} />
            ...
            <div className="relative min-h-full">
              ...
              {drawingState.stickyNotes.map((note) => (
                <StickyNote
                  ...
                  onDelete={() => handleStickyNotesChange(drawingState.stickyNotes.filter((n) => n.id !== note.id))}
                />
              ))}

              {drawingState.mathSuggestions.map((suggestion) => (
                <FloraSuggestionChip
                  key={suggestion.id}
                  suggestion={suggestion}
                  onAccept={() => {
                    handleMathSuggestionsChange(drawingState.mathSuggestions.map((s) => s.id === suggestion.id ? { ...s, accepted: true } : s));
                    handleContentChange(`${page?.content || ""}<p>${suggestion.result}</p>`);
                  }}
                  onDismiss={() => handleMathSuggestionsChange(drawingState.mathSuggestions.filter((s) => s.id !== suggestion.id))}
                />
              ))}
            </div>
          </div>
              ...
              <div>
                ...
                <div className="grid gap-2">
                  {quizQuestions[quizIndex].alternativas.map((alt, idx) => (
                    <Button
                      key={idx}
                      variant={quizSelected === idx ? (idx === quizQuestions[quizIndex].correta ? "success" : "destructive") : "outline"}
