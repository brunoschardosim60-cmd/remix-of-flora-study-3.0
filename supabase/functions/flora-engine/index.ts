...
SEMPRE responda em português brasileiro.` },
    ...
    if (action === "generate_lesson") {
      ...
      const topic = data?.topic || "tema geral";
      const materia = data?.materia || "Geral";
      const level = data?.level || "enem";
      const didacticStyle = data?.didacticStyle || "normal";
      const content = data?.content || topic;

      const { LESSON_SYSTEM_PROMPT, buildLessonPrompt } = await import("../_shared/prompts_aulas.ts");
      const context = await getStudentContext(userId);
      const personality: FloraPersonality = (context?.onboarding?.flora_personality as FloraPersonality) || "padrao";
      const explanationStyle: ExplanationStyle = (context?.onboarding?.explanation_style as ExplanationStyle) || "padrao";
      ...
      await supabase.from("user_actions").insert({ user_id: userId, action: "generate_lesson", materia, metadata: { topic, level, didacticStyle } });
      return jsonResponse({ ok: true, lesson: parsedLesson });
    }
